...
export const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      playSound('success');
      notify.success('تم تسجيل الدخول بنجاح إلى متجر سوبر ماركت العباسي!');
    } catch (err: any) {
  ...
  const handlePrefillAdmin = (e?: React.MouseEvent) => {
    ...
    setEmail('Mohammedalsarem6@gmail.com');
      ...
      <div className="w-full max-w-md bg-white border border-[#EBE3DB] rounded-[2.5rem] p-8 shadow-2xl space-y-6 relative overflow-hidden">
        ...
        <div className="text-center relative z-10 flex flex-col items-center">
          <div className="w-20 h-20 rounded-[1.5rem] bg-[#180F0A] border-2 border-[#E2A85C]/30 flex items-center justify-center text-white mb-4 shadow-xl shadow-amber-500/10 overflow-hidden">
            <img src={defaultAppIcon} alt="نظام سوبر ماركت العباسي" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
          </div>
          <h2 className="text-2xl font-black text-[#5E1A1A] tracking-tight">بوابة نظام سوبر ماركت العباسي</h2>
