...
export const drawInvoiceOnCanvas = async (invoice: any, shopSettings: any): Promise<HTMLCanvasElement> => {
  ...
  ctx.fillText(shopSettings?.shopName || 'سوبر ماركت العباسي', 750, 75);
  ...
  if (logoImg && logoImg.complete && logoImg.width > 0) {
    ...
    ctx.fillText('نظام سوبر ماركت العباسي للمبيعات', 0, 0);
  }
  ctx.restore();
  ...
  ctx.fillText(`تم استخراج هذه الفاتورة بصيغة PDF إلكترونياً من نظام ${shopSettings?.shopName || 'سوبر ماركت العباسي'}`, 400, systemLineY);
