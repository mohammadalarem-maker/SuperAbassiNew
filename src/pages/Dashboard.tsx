...
const getCurrencyValue = (dataObj: { [code: string]: number } | undefined, currencyCode: 'YER' | 'USD' | 'SAR'): number => {
  if (!dataObj) return 0;
  
  if (dataObj[currencyCode] !== undefined) {
    return dataObj[currencyCode];
  }
  
  const lowerTarget = currencyCode.toLowerCase();
  ...
  for (const key of keys) {
    ...
    if (lowerTarget === 'yer') {
      if (lowerKey === 'yer' || lowerKey === 'ر.ي' || lowerKey.includes('يمني') || lowerKey.includes('yemen')) {
        return dataObj[key];
      }
    }
    if (lowerTarget === 'sar') {
      if (lowerKey === 'sar' || lowerKey === 'ر.س' || lowerKey.includes('سعودي') || lowerKey.includes('saudi')) {
        return dataObj[key];
      }
    }
    if (lowerTarget === 'usd') {
      if (lowerKey === 'usd' || lowerKey === '$' || lowerKey.includes('دولار') || lowerKey.includes('dollar')) {
        return dataObj[key];
      }
    }
...
export default function Dashboard() {
  ...
  // Derived stats
  const usdRate = (() => {
    if (shopSettings?.currencies && Array.isArray(shopSettings.currencies)) {
      const usdObj = shopSettings.currencies.find((c: any) => c.code === 'USD');
      if (usdObj && usdObj.rate) return Number(usdObj.rate);
    }
    return 530; // Fallback
  })();
  ...
  const sarRate = (() => {
    if (shopSettings?.currencies && Array.isArray(shopSettings.currencies)) {
      const sarObj = shopSettings.currencies.find((c: any) => c.code === 'SAR');
      if (sarObj && sarObj.rate) return Number(sarObj.rate);
    }
    return 140; // Fallback
  })();
  ...
  const convertToYer = (value: number, curr?: string) => {
    ...
    if (curr === 'USD') return val * usdRate;
    if (curr === 'SAR') return val * sarRate;
    return val;
  };

  const totalSales = invoices.reduce((sum, inv) => sum + convertToYer(inv.total || 0, inv.currency), 0);
  ...
  const grossProfit = invoices.reduce((sum, inv) => {
    ...
    return sum + convertToYer(invProfit, inv.currency);
  }, 0);

  const totalExpensesSum = expenses.reduce((sum, d) => sum + (d.amount || 0), 0);
  const netProfit = grossProfit - totalExpensesSum;

  const activeCurrencies = shopSettings?.currencies || [
    { code: shopSettings?.currency || 'ر.ي', symbol: shopSettings?.currency || 'ر.ي', rate: 1, isBase: true },
    { code: 'USD', symbol: '$', rate: usdRate, isBase: false },
    { code: 'SAR', symbol: 'ر.س', rate: sarRate, isBase: false }
  ];

  const totalSalesByCurrency: { [code: string]: number } = {};
  const netProfitByCurrency: { [code: string]: number } = {};

  activeCurrencies.forEach((curr: any) => {
    totalSalesByCurrency[curr.code] = 0;
    netProfitByCurrency[curr.code] = 0;
  });
  ...
  invoices.forEach((inv) => {
    ...
    const revenueInYer = convertToYer(revenue, inv.currency);
    const profitInYer = convertToYer(profit, inv.currency);

    activeCurrencies.forEach((curr: any) => {
      if (curr.isBase) {
        totalSalesByCurrency[curr.code] += revenueInYer;
        netProfitByCurrency[curr.code] += profitInYer;
      } else {
        if (inv.currencyEquivalents && inv.currencyEquivalents[curr.code] !== undefined) {
          totalSalesByCurrency[curr.code] += inv.currencyEquivalents[curr.code];
          const rateAtSale = inv.currencyRatesAtSale?.find((c: any) => c.code === curr.code)?.rate || curr.rate;
          netProfitByCurrency[curr.code] += rateAtSale > 0 ? parseFloat((profitInYer / rateAtSale).toFixed(2)) : 0;
        } else {
          totalSalesByCurrency[curr.code] += curr.rate > 0 ? parseFloat((revenueInYer / curr.rate).toFixed(2)) : 0;
          netProfitByCurrency[curr.code] += curr.rate > 0 ? parseFloat((profitInYer / curr.rate).toFixed(2)) : 0;
        }
      }
    });
  });
  ...
  activeCurrencies.forEach((curr: any) => {
    if (curr.isBase) {
      netProfitByCurrency[curr.code] -= totalExpensesSum;
    } else {
      const currentRate = curr.rate || 1;
      netProfitByCurrency[curr.code] -= currentRate > 0 ? parseFloat((totalExpensesSum / currentRate).toFixed(2)) : 0;
    }
  });
  ...
  const totalInventoryValueByCurrency: { [code: string]: number } = { YER: 0, SAR: 0, USD: 0 };
  items.forEach((item: any) => {
    const itemCurrency = item.currency || 'YER';
    const purchasePrice = Number(item.purchasePrice) || Number(item.price) * 0.70 || 0;
    const stock = Number(item.stock) || 0;
    const value = purchasePrice * stock;
    if (totalInventoryValueByCurrency[itemCurrency] !== undefined) {
      totalInventoryValueByCurrency[itemCurrency] += value;
    } else {
      totalInventoryValueByCurrency[itemCurrency] = value;
    }
  });
  ...
  const dailySales = todayInvoices.reduce((sum, inv) => sum + convertToYer(inv.total || 0, inv.currency), 0);
  ...
  const monthlySalesAmount = monthInvoices.reduce((sum, inv) => sum + convertToYer(inv.total || 0, inv.currency), 0);
  ...
  const getInteractiveChartData = (daysCount: number) => {
    ...
    for (let i = daysCount - 1; i >= 0; i--) {
      ...
      const daySales = dayInvoices.reduce((sum, inv) => sum + convertToYer(inv.total || 0, inv.currency), 0);
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-6 text-right" id="dashboard-content" dir="rtl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 print:hidden p-6 -mx-6 -mt-6 bg-gradient-to-r from-[#1a1a3a] via-[#3b154d] to-[#1a1a3a] rounded-b-[2.5rem] shadow-xl text-white">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-primary">مرحباً بك في {shopSettings?.shopName || 'سوبر ماركت العباسي'}</h1>
        ...
        <div className="flex gap-2 md:gap-3">
           ...
           <button onClick={() => navigate('/pos')} className="flex-1 sm:flex-none bg-gradient-to-r from-orange-400 via-pink-500 to-purple-600 text-white border-0 shadow-lg text-xs md:text-sm py-2 px-4 whitespace-nowrap cursor-pointer">
             فاتورة جديدة +
           </button>
        </div>
      </div>
      ...
      {/* 3 prominent separate statistical boxes for currencies */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 animate-fade-in text-right" dir="rtl">
        {/* YER Box */}
        <motion.div 
          whileHover={{ y: -4 }}
          className="bg-gradient-to-br from-[#8B5E3C]/10 to-[#8B5E3C]/5 dark:from-[#8B5E3C]/20 dark:to-slate-900 border border-[#8B5E3C]/20 p-5 rounded-2xl relative overflow-hidden shadow-xs"
        >
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              <span className="px-2.5 py-1 bg-[#8B5E3C]/15 text-[#8B5E3C] text-[10px] font-black rounded-lg">الريال اليمني (YER)</span>
              <h3 className="text-xl md:text-2xl font-black text-gray-900 dark:text-white pt-2.5">
                {getCurrencyValue(totalSalesByCurrency, 'YER').toLocaleString(undefined, { maximumFractionDigits: 2 })} <span className="text-xs text-gray-500 font-bold">ر.ي</span>
              </h3>
              <p className="text-[10px] text-gray-400 font-bold">إجمالي المبيعات بالعملة</p>
            </div>
            <div className="w-10 h-10 rounded-xl bg-[#8B5E3C]/15 text-[#8B5E3C] flex items-center justify-center font-black text-sm">
              YER
            </div>
          </div>
          <div className="mt-4 pt-3 border-t border-dashed border-[#8B5E3C]/20 flex justify-between items-center text-xs">
            <span className="text-gray-400 font-bold">إجمالي المخزون (شراء):</span>
            <span className="font-extrabold text-[#8B5E3C]">
              {getCurrencyValue(totalInventoryValueByCurrency, 'YER').toLocaleString(undefined, { maximumFractionDigits: 0 })} YER
            </span>
          </div>
        </motion.div>
        ...
        >
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              <span className="px-2.5 py-1 bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 text-[10px] font-black rounded-lg">الريال السعودي (SAR)</span>
              <h3 className="text-xl md:text-2xl font-black text-gray-900 dark:text-white pt-2.5">
                {getCurrencyValue(totalSalesByCurrency, 'SAR').toLocaleString(undefined, { maximumFractionDigits: 2 })} <span className="text-xs text-gray-500 font-bold">ر.س</span>
              </h3>
              <p className="text-[10px] text-gray-400 font-bold">إجمالي المبيعات بالعملة</p>
            </div>
          ...
          <div className="mt-4 pt-3 border-t border-dashed border-emerald-500/20 flex justify-between items-center text-xs">
            <span className="text-gray-400 font-bold">إجمالي المخزون (شراء):</span>
            <span className="font-extrabold text-emerald-600 dark:text-emerald-400">
              {getCurrencyValue(totalInventoryValueByCurrency, 'SAR').toLocaleString(undefined, { maximumFractionDigits: 0 })} SAR
            </span>
          </div>
        </motion.div>
        ...
        <motion.div 
          whileHover={{ y: -4 }}
          className="bg-gradient-to-br from-blue-500/10 to-blue-500/5 dark:from-blue-950/20 dark:to-slate-900 border border-blue-500/20 p-5 rounded-2xl relative overflow-hidden shadow-xs"
        >
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              <span className="px-2.5 py-1 bg-blue-500/15 text-blue-600 dark:text-blue-400 text-[10px] font-black rounded-lg">الدولار الأمريكي (USD)</span>
              <h3 className="text-xl md:text-2xl font-black text-gray-900 dark:text-white pt-2.5">
                {getCurrencyValue(totalSalesByCurrency, 'USD').toLocaleString(undefined, { maximumFractionDigits: 2 })} <span className="text-xs text-gray-500 font-bold">$</span>
              </h3>
              <p className="text-[10px] text-gray-400 font-bold">إجمالي المبيعات بالعملة</p>
            </div>
            <div className="w-10 h-10 rounded-xl bg-blue-500/15 text-blue-600 flex items-center justify-center font-black text-sm">
              USD
            </div>
          </div>
          <div className="mt-4 pt-3 border-t border-dashed border-blue-500/20 flex justify-between items-center text-xs">
            <span className="text-gray-400 font-bold">إجمالي المخزون (شراء):</span>
            <span className="font-extrabold text-blue-600 dark:text-blue-400">
              {getCurrencyValue(totalInventoryValueByCurrency, 'USD').toLocaleString(undefined, { maximumFractionDigits: 0 })} USD
            </span>
          </div>
        </motion.div>
      </div>

      {shopSettings?.multiCurrencyActive && (
         <motion.div
           initial={{ opacity: 0, y: 15 }}
           animate={{ opacity: 1, y: 0 }}
           className="bg-surface border border-gray-100 dark:border-slate-800 rounded-2xl p-5 md:p-6 shadow-xs space-y-4 text-right animate-fade-in"
         >
           <div className="flex items-center justify-between border-b border-gray-100 dark:border-slate-800 pb-3">
             <div className="flex items-center gap-2">
               ...
               <div>
                  <h3 className="text-sm font-black text-gray-900 dark:text-white font-sans">المبيعات والأرباح بالعملات الأجنبية 💸</h3>
                  <p className="text-[10px] text-gray-550 font-bold font-sans">ملخص مبيعات وصافي أرباح المحل بمختلف العملات وأسعار الصرف الحالية</p>
               </div>
             </div>
             <span className="px-2.5 py-1 bg-emerald-500/10 text-emerald-600 text-[10px] font-extrabold rounded-md font-sans">الربط المالي نشط</span>
           </div>
                 ...
                 return (
                    <div key={curr.code} className="bg-gray-50/50 dark:bg-[#111827]/50 border border-gray-150/40 dark:border-slate-800/80 rounded-xl p-4 space-y-2 relative overflow-hidden">
                       <div className="flex justify-between items-center bg-gray-100/35 dark:bg-slate-800/40 px-2 py-1 rounded-lg">
                          ...
                          {!curr.isBase && (
                             <span className="text-[9px] text-[#E2A85C] font-mono font-extrabold">1 = {curr.rate} {shopSettings?.currency || 'ر.ي'}</span>
                          )}
                       </div>
                       
                       <div className="space-y-1 pt-1 font-sans">
                          <div className="flex justify-between items-center text-xs">
                             <span className="text-gray-400 font-bold">إجمالي المبيعات:</span>
                             <span className="font-mono font-black text-gray-900 dark:text-white">
                                 {totalS.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 2 })} {curr.symbol}
                             </span>
                          </div>
                          <div className="flex justify-between items-center text-xs pt-1 border-t border-dashed border-gray-200/50 dark:border-slate-800">
                             <span className="text-gray-400 font-bold">صافي الأرباح:</span>
                             <span className={`font-mono font-black ${netP >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-500'}`}>
                                 {netP.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 2 })} {curr.symbol}
                             </span>
                          </div>
                       </div>
                    </div>
      ...
      <div className="space-y-4 text-right" dir="rtl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          ...
          >
            ...
            {/* Custom 3D Icon */}
            <div className="relative shrink-0 w-20 h-20 md:w-24 md:h-24 rounded-2xl bg-neutral-900/5 dark:bg-slate-800/40 flex items-center justify-center p-1 overflow-hidden transition-transform duration-500 group-hover:scale-105 group-hover:rotate-1">
              <img 
                src={inventoryIcon} 
                alt="إدارة المخازن" 
      ...
      <div className="pt-8 pb-4 text-center text-xs text-secondary border-t border-gray-100 dark:border-slate-800/55 mt-8">
         <p>تطوير المهندس: <span className="font-bold text-gray-700 dark:text-gray-300">محمد الصارم</span></p>
      </div>
    </div>
  );
}
