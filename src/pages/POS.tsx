...
const getCurrencySymbol = (currencyCode: string | undefined): string => {
  ...
  if (currencyCode === 'YER') return 'ر.ي';
...
export default function POS() {
  ...
  const handleBluetoothPrint = async () => {
    ...
    try {
      await BluetoothPrinterService.printInvoice(lastInvoice, shopSettings?.shopName || 'سوبر ماركت العباسي');
  ...
  const completeSale = async () => {
    ...
    try {
      const invoiceResult = await runTransaction(db, async (transaction) => {
        ...
        const saleCurrency = cart[0]?.item?.currency || 'YER';
        ...
        const currencies = shopSettings?.currencies || [
        ...
        const currencyEquivalents: { [code: string]: number } = {};
        currencies.forEach((curr: any) => {
          if (curr.rate > 0) {
            if (curr.isBase) {
              currencyEquivalents[curr.code] = grandTotal;
            } else {
              currencyEquivalents[curr.code] = parseFloat((grandTotal / curr.rate).toFixed(2));
            }
          }
        });
      ...
      const finalSaleCurrency = cart[0]?.item?.currency || 'YER';
          ...
          >
             ...
             <div className="hidden print:block fixed inset-0 bg-white p-8 text-black text-right relative" dir="rtl" id="pos-receipt-print">
                ...
                <div className="text-center mb-6 max-w-sm mx-auto relative z-10">
                   ...
                   />
                   <h2 className="text-xl font-black">{shopSettings?.shopName || 'سوبر ماركت العباسي'}</h2>
           ...
           <div className="flex items-center gap-2">
             <div className="relative flex-1 group">
               ...
               <input 
                 ...
                 type="text" 
                 placeholder="ابحث..." 
                 className="w-full bg-background dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg pr-9 md:pr-10 pl-4 py-2 text-xs md:text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-foreground text-right" 
                 value={searchTerm}
             ...
             <button 
               ...
               className="p-2 md:p-2.5 bg-secondary/10 text-secondary rounded-lg hover:bg-secondary/20 transition-all flex items-center gap-2 border border-secondary/20 shadow-sm cursor-pointer"
               title="الباركود بالكاميرا"
             >
