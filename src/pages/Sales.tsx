...
export default function SalesHistory() {
  ...
  const getCurrencySymbol = (currencyCode: string | undefined): string => {
    if (currencyCode === 'USD' || currencyCode === '$') return '$';
    if (currencyCode === 'SAR' || currencyCode === 'ر.س') return 'ر.س';
    if (currencyCode === 'YER' || currencyCode === 'ر.ي') return 'ر.ي';
    return currencyCode || 'ر.ي';
  };

  const getExchangeRate = (currencyCode: string | undefined): number => {
    if (!currencyCode || currencyCode === 'YER' || currencyCode === 'ر.ي') return 1;
    const found = shopSettings?.currencies?.find((c: any) => c.code === currencyCode || c.symbol === currencyCode);
    if (found && found.rate > 0) return Number(found.rate);
    if (currencyCode === 'USD' || currencyCode === '$') return 530;
    if (currencyCode === 'SAR' || currencyCode === 'ر.س') return 140;
    return 1;
  };

  const getInvoiceTotalInYer = (inv: Invoice | any): number => {
    const rawTotal = inv.total || 0;
    const invCurrency = inv.currency || 'YER';
    const rate = getExchangeRate(invCurrency);
  ...
  const handlePrintBleReceiptDirectly = async (target: Invoice) => {
    ...
    try {
      await BluetoothPrinterService.printInvoice(target, shopSettings?.shopName || 'سوبر ماركت العباسي');
